import React, {useEffect, useState} from 'react';
import {
  TextInput,
  View,
  ScrollView,
  StyleSheet,
  Image,
  Dimensions,
  TouchableOpacity,
  Text,
} from 'react-native';
import Category from './category/Category';
import Recommendation from './component/Recommendation';
import FrequentlyPurchasedItem from './component/FrequentlyPurchasedItem';
import WhereYouLeft from './component/WhereYouLeft';
import Discounted from './component/Discounted';
import BrowsingHistory from './component/BrowsingHistory';
import TrendingDailyNeeds from './component/TrendingDailyNeeds';
import TrendingLikedItems from './component/TrendingLikedItems';
import TopPics from './component/TopPics';

const {width} = Dimensions.get('window');
const productData = require('../../collections/trending.json');
function homescreen({navigation, route}) {
  const handleClick = () => {
    console.log('Product Item Yop Pics');
  };

  const handleItemClick = item => {
    console.log('Product Item', item);
    navigation.navigate('ProductDetails', {PRODUCTITEM: item.item});
  };

  const handleCategoryItemClick = value => {
    navigation.navigate('CategoryBasedProducs', {CATEGORY: value});
  };

  const handleBuyNow = item =>
    navigation.navigate('Payment', {PRODUCTITEM: item.item});

  const handleAddToCart = item => {
    getpostEvents(item);
  };

  const getpostEvents = async item => {
    let response = await fetch(
      `http://localhost:3002/api/postEvents/${item.item.ITEM_ID}/${UserId}/ADD TO CART`,
    );
    let jsonData = await response.json();
    console.log('Post Event Json Data', jsonData);
  };

  return (
    <View>
      <View
        style={{
          flexDirection: 'row',
          justifyContent: 'space-between',
          backgroundColor: 'white',
          height: 50,
        }}>
        <View style={{justifyContent: 'center', marginLeft: 10}}>
          <Text
            style={{
              fontWeight: '900',
              color: '#A1560B',
              fontSize: 22,
              letterSpacing: 0.8,
            }}>
            Welcome
          </Text>
        </View>
        <View
          style={{
            flexDirection: 'row',
            justifyContent: 'center',
            marginRight: 10,
          }}>
          <TouchableOpacity
            onPress={() => {
              navigation.navigate('Transaction');
            }}
            style={{
              alignSelf: 'center',
            }}>
            <Text
              style={{
                alignSelf: 'center',
                fontWeight: '500',
                color: 'grey',
                marginRight: 10,
                fontSize: 20,
              }}>
              Transaction
            </Text>
          </TouchableOpacity>

          <Image
            style={{
              width: 15,
              height: 15,
              alignSelf: 'center',
              alignItems: 'center',
              justifyContent: 'center',
            }}
            source={{
              uri: 'data:image/png;base64,',
            }}
          />
        </View>
      </View>
      <View style={{flexDirection: 'row', alignItems: 'center'}}>
        <View
          style={{
            height: 40,
            borderWidth: 1,
            borderColor: '#D3D3D3',
            margin: 10,
            padding: 10,
            flex: 1.8,
            borderRadius: 10,
          }}>
          <TextInput
            placeholderTextColor={'#D3D3D3'}
            placeholder={'Search here...'}
            style={{
              height: 40,
              paddingStart: 15,
              position: 'absolute',
              marginLeft: 10,
            }}
          />
          <Image
            style={{
              width: 15,
              height: 15,
              position: 'absolute',
              top: 12,
              marginLeft: 10,
            }}
            source={{
              uri: 'data:image/png;base64,',
            }}
          />
        </View>
      </View>

      <ScrollView
        style={{
          width: '100%',
          height: '100%',
        }}>
        <Category handleCategoryItemClick={handleCategoryItemClick} />
        <View>
          <Text style={styles.heading}>
            Frequently repurchased items for you
          </Text>
          <FrequentlyPurchasedItem
            handleBuyNow={handleBuyNow}
            handleItemClick={handleItemClick}
          />
        </View>

        <View style={{marginTop: 20}}>
          <Text style={styles.heading}>Recommendation for you</Text>
          <Recommendation
            handleAddToCart={handleAddToCart}
            handleItemClick={handleItemClick}
          />
        </View>

        <View style={{marginTop: 20}}>
          <Text style={styles.heading}>Pick from where you left</Text>
          <WhereYouLeft
            handleBuyNow={handleBuyNow}
            handleItemClick={handleItemClick}
          />
        </View>

        <View style={{marginTop: 20}}>
          <Text style={styles.heading}>Discounted for you</Text>
          <Discounted
            handleBuyNow={handleBuyNow}
            handleItemClick={handleItemClick}
          />
        </View>

        <View style={{marginTop: 20}}>
          <Text style={styles.heading}>Inspired by your browsing history</Text>
          <BrowsingHistory
            handleAddToCart={handleAddToCart}
            handleItemClick={handleItemClick}
          />
        </View>

        <View style={{marginTop: 20}}>
          <Text style={styles.heading}>Top Pics:- </Text>
          <TopPics handleClick={handleClick}></TopPics>
        </View>

        <View style={{marginTop: 20}}>
          <Text style={styles.heading}>Trending Daily Needs</Text>
          <TrendingDailyNeeds
            handleAddToCart={handleAddToCart}
            handleItemClick={handleItemClick}
          />
        </View>

        <View style={{marginTop: 20, paddingBottom: 180}}>
          <Text style={styles.heading}>Trending Items you might also like</Text>
          <TrendingLikedItems
            handleAddToCart={handleAddToCart}
            handleItemClick={handleItemClick}
          />
        </View>
      </ScrollView>
    </View>
  );
}

const styles = StyleSheet.create({
  row: {
    flex: 1,
    flexDirection: 'row',
    justifyContent: 'center',
  },
  col: {
    flex: 1,
  },
  inner: {
    borderWidth: 2,
    borderColor: 'red',
    margin: 8,
  },
  container: {flex: 1, backgroundColor: 'white'},
  child: {width, justifyContent: 'center'},
  text: {fontSize: width * 0.5, textAlign: 'center'},
  heading: {
    fontSize: 22,
    fontWeight: '700',
    marginLeft: 10,
    padding: 5,
    color: 'black',
    letterSpacing: 0.2,
    borderBottomColor: '#e2e2e2',
    borderBottomWidth: 1,
    borderBottomLength: 60,
  },
});

export default homescreen;
